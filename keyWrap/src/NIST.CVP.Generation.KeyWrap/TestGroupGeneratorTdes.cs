...
{
    public class TestGroupGeneratorTdes : ITestGroupGenerator<ParametersTdes>
    {
        public const int _MAX_BIT_SIZE = 4096;

        public IEnumerable<ITestGroup> BuildTestGroups(ParametersTdes parameters)
        {
        ...
        private void CreateGroups(List<ITestGroup> groups, ParametersTdes parameters)
        {
            ...
            foreach (var direction in parameters.Direction)
            {
                foreach (var kwCipher in parameters.KwCipher)
                {
                    foreach (var ptLen in testPtLens)
                    {
                        ...
                        TestGroupTdes testGroup = new TestGroupTdes()
                        {
                            //KeyWrapType = keyWrapType,
                            Direction = direction,
                            PtLen = ptLen,

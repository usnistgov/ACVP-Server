...
namespace NIST.CVP.Generation.DSA.ECC.KeyGen.IntegrationTests
{
    ...
    public class GenValTests
    {
        ...
        public void ValShouldReturn1OnFailedRun()
        {
            ...
            var result = ECDSA_KeyGen_Val.Program.Main(
        ...
        private List<int> DoBadThingsToResultsFile(string resultsFile)
        {
            ...
            foreach (var testCase in parsedValidation.ParsedObject.testResults)
            {
                if ((int)testCase.tcId % 2 == 0)
                {
                    ...
                    if (testCase.qx != null)
                    {
                        testCase.qx = rand.GetDifferentBitStringOfSameSize(new BitString((string)testCase.qx)).ToHex();
                        continue;
                    }

                    if (testCase.qy != null)
                    {
                        testCase.qy = rand.GetDifferentBitStringOfSameSize(new BitString((string)testCase.qy)).ToHex();
                        continue;
                    }
                }
            }

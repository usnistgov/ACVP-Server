...
namespace NIST.CVP.Generation.DSA.ECC.KeyVer.IntegrationTests
{
    ...
    public class GenValTests
    {
        ...
        public void ValShouldReturn1OnFailedRun()
        {
            ...
            var result = ECDSA_KeyVer_Val.Program.Main(
        ...
        private string GetTestFolder(string name = "")
        {
            ...
            var folderName = "KeyVer--" + prefix + Guid.NewGuid().ToString().Substring(0, 8);
        ...
        private List<int> DoBadThingsToResultsFile(string resultsFile)
        {
            ...
            foreach (var testCase in parsedValidation.ParsedObject.testResults)
            {
                if ((int)testCase.tcId % 2 == 0)
                {
                    ...
                    {
                        var currentValue = EnumHelpers.GetEnumFromEnumDescription<Disposition>(((string)testCase.result).ToLower());
                        testCase.result = currentValue == Disposition.Passed ? EnumHelpers.GetEnumDescriptionFromEnum(Disposition.Failed) : EnumHelpers.GetEnumDescriptionFromEnum(Disposition.Passed);
                    }
                }
            }

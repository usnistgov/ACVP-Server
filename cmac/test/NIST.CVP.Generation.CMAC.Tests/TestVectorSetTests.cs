...
namespace NIST.CVP.Generation.CMAC_AES.Tests
{
    ...
    public class TestVectorSetTests
    {
        ...
        [Test]
        public void GenShouldIncludeMsgInAnswerProjection()
        {
            var subject = GetSubject(1);
            var results = subject.AnswerProjection;
            var group = results[0];
        ...
        [Test]
        public void GenShouldIncludeMsgInPromptProjection()
        {
            var subject = GetSubject(1);
            var results = subject.PromptProjection;
            var group = results[0];
        ...
        [Test]
        public void VerShouldIncludeMacInAnswerProjection()
        {
            var subject = GetSubject(1, "ver");
            var results = subject.AnswerProjection;
            var group = results[0];
            var tests = group.tests;
            foreach (var test in tests)
            {
                Assert.IsTrue((bool) !string.IsNullOrEmpty(test.mac.ToString()));
            }
        }

        [Test]
        public void VerShouldIncludeMacInPromptProjection()
        {
            var subject = GetSubject(1, "ver");
            var results = subject.PromptProjection;
            var group = results[0];
        ...
        private TestVectorSet GetSubject(int groups = 1, string direction = "gen", bool failureTest = false)
        {
            var subject = new TestVectorSet {Algorithm = "CMAC-AES-128"};

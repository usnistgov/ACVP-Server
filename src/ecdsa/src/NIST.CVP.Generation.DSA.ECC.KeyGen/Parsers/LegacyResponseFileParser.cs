...
namespace NIST.CVP.Generation.DSA.ECC.KeyGen.Parsers
{
    public class LegacyResponseFileParser : ILegacyResponseFileParser<TestVectorSet>
    {
        public ParseResponse<TestVectorSet> Parse(string path)
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (workingLine.StartsWith("["))
                {
                    ...
                    foreach (var prop in propParts)
                    {
                        ...
                        {
                            curGroup.SetString("curve", valuePair[0].Trim());
                        }
                        else
                        {
            ...
            return new ParseResponse<TestVectorSet>(new TestVectorSet { Algorithm = "ECDSA", Mode = "KeyGen", TestGroups = groups.Select(g => (ITestGroup)g).ToList() });
        }
    }
}

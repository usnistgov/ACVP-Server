...
namespace NIST.CVP.Generation.TupleHash.Tests
{
    ...
    public class TestCaseGeneratorAFTHashTests
    {
        ...
        [TestCase(128, true, true)]
        [TestCase(256, false, false)]
        public void ShouldGenerateProperlySizedBitOrientedMessageForEachGenerateCallForEachRate(int digestSize, bool xof, bool includeNull)
        {
            var mockSHA = new Mock<ITupleHash>();
            mockSHA.Setup(s => s.HashMessage(It.IsAny<HashFunction>(), It.IsAny<List<BitString>>()))
                .Returns(new HashResult(new BitString("ABCD")));
            ...
            for (var caseIdx = 0; caseIdx < subject.NumberOfTestCasesToGenerate; caseIdx++)
            {
                ...
                if (includeNull)
                {
                    if (empty < 10)
                    {
                        ...
                        {
                            Assert.AreEqual(0, element.BitLength);
                        }
                        empty++;
                    }
                    else if (semiEmpty < 30)
                    ...
                    {
                        Assert.AreEqual(small++, testCase.Tuple.ElementAt(0).BitLength);
                    }
                    else if (nonEmpty <= 25)
                    {
                        foreach (var element in testCase.Tuple)
                        {
                            Assert.IsTrue(element.BitLength > 0 && element.BitLength < 513);
                        }
                        nonEmpty++;
                    }
                    else
                    {
                        Assert.AreEqual(rate + currentLargeCase++ * (rate + 1), testCase.Tuple.ElementAt(0).BitLength);
                    }
                }
                else
                {
        ...
        [TestCase(128, true, true)]
        [TestCase(256, false, false)]
        public void ShouldGenerateProperlySizedByteOrientedMessageForEachGenerateCallForEachRate(int digestSize, bool xof, bool includeNull)
        {
            var mockSHA = new Mock<ITupleHash>();
            mockSHA.Setup(s => s.HashMessage(It.IsAny<HashFunction>(), It.IsAny<List<BitString>>()))
                .Returns(new HashResult(new BitString("ABCD")));
            ...
            for (var caseIdx = 0; caseIdx < subject.NumberOfTestCasesToGenerate; caseIdx++)
            {
                ...
                if (includeNull)
                {
                    ...
                    else if (small <= ((1600 - (digestSize * 2)) * 2) / 8)
                    {
                        Assert.AreEqual(small++ * 8, testCase.Tuple.ElementAt(0).BitLength);
                    }
                    else if (nonEmpty <= 25)
                    {
                        foreach (var element in testCase.Tuple)
                    ...
                    {
                        Assert.AreEqual(rate + currentLargeCase++ * (rate + 8), testCase.Tuple.ElementAt(0).BitLength);
                    }
                }
                else
                {
                    if (small <= (1600 - digestSize * 2) * 2 / 8)
                    {
                        Assert.AreEqual(small++ * 8, testCase.Tuple.ElementAt(0).BitLength);
                    }
                    else if (nonEmpty <= 25)

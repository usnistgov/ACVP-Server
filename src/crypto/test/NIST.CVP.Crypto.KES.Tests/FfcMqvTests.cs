using NIST.CVP.Crypto.Common.Asymmetric.DSA.FFC;
using NIST.CVP.Math;
using NIST.CVP.Math.Helpers;
using NIST.CVP.Tests.Core.TestCategoryAttributes;
using NUnit.Framework;

namespace NIST.CVP.Crypto.KES.Tests
{
    [TestFixture, FastCryptoTest]
    public class FfcMqvTests
    {
        private MqvFfc _subject;

        [SetUp]
        public void Setup()
        {
            _subject = new MqvFfc();
        }

        private static object[] SuccessSharedSecretZTests = new object[]
        {
            new object[]
            {
                "from CAVS test1",
                // xPrivateKeyPartyA
                new BitString("95b0c5c27323a5db027b3e5cc147401a6e14f74140a1e0abb3c97c54"),
                // yPublicKeyPartyB
                new BitString(""),
                // rPrivateKeyPartyA
                new BitString("54d2a8339aa26a7e5c43cf59fdbecb554a3d52b4e9fbaba22f0c3f1c"),
                // tPublicKeyPartyA
                new BitString(""),
                // tPublicKeyPartyB
                new BitString(""),
                // P
                new BitString(""),
                // Q
                new BitString("f350ea98cf1f0049178f8ee9061746c92f1182da3f620b368260a0b1"),
                // Expected Z
                new BitString(""),
            },
            new object[]
            {
                "from CAVS test2",
                // xPrivateKeyPartyA
                new BitString("a5c5f77df6d5e78340852dd3df08c9b465a7670c7872dc72b84bc600"),
                // yPublicKeyPartyB
                new BitString(""),
                // rPrivateKeyPartyA
                new BitString("17ea24e7ad1cf773db5f8617e01b16a9a6ff4aad1f475246bcd9f375"),
                // tPublicKeyPartyA
                new BitString(""),
                // tPublicKeyPartyB
                new BitString(""),
                // P
                new BitString(""),
                // Q
                new BitString("b7a1971219418d90e12bc41a912fe36e7cc316397e66b4f1fbe44969"),
                // Expected Z
                new BitString(""),
            },
            new object[]
            {
                "from CAVS test3",
                // xPrivateKeyPartyA
                new BitString("69dd82efcf663d211bf3e70e2d58f18e0b688c4b61f0feb7cd87eb5b"),
                // yPublicKeyPartyB
                new BitString(""),
                // rPrivateKeyPartyA
                new BitString("1168b4abe59c0117d42a987f0642ca0c7b2c3431fd4faea3627356b9"),
                // tPublicKeyPartyA
                new BitString(""),
                // tPublicKeyPartyB
                new BitString(""),
                // P
                new BitString(""),
                // Q
                new BitString("8b0961c1a8c1c3cd6ac715c20ebe036325f57c94f40c308fdb87890f"),
                // Expected Z
                new BitString(""),
            },
            new object[]
            {
                "from CAVS test4",
                // xPrivateKeyPartyA
                new BitString("20de34509cc419662981335cf1b697a2c5231fd12cde6207fde008e2"),
                // yPublicKeyPartyB
                new BitString(""),
                // rPrivateKeyPartyA
                new BitString("6e1e4a900c0094eb95a3da95c4bf233dec8fc58298be7248be1d9296"),
                // tPublicKeyPartyA
                new BitString(""),
                // tPublicKeyPartyB
                new BitString(""),
                // P
                new BitString(""),
                // Q
                new BitString("8b0961c1a8c1c3cd6ac715c20ebe036325f57c94f40c308fdb87890f"),
                // Expected Z
                new BitString(""),
            }
            //new object[]
            //{
            //    "from CAVS test1",
            //    // xPrivateKeyPartyA
            //    new BitString(""),
            //    // yPublicKeyPartyB
            //    new BitString(""),
            //    // rPrivateKeyPartyA
            //    new BitString(""),
            //    // tPublicKeyPartyA
            //    new BitString(""),
            //    // tPublicKeyPartyB
            //    new BitString(""),
            //    // P
            //    new BitString(""),
            //    // Q
            //    new BitString(""),
            //    // Expected Z
            //    new BitString(""),
            //}
        };

        [Test]
        [TestCaseSource(nameof(SuccessSharedSecretZTests))]
        public void ShouldCalculateCorrectSharedSecret(
            string label,
            BitString xPrivateKeyPartyA,
            BitString yPublicKeyPartyB,
            BitString rPrivateKeyPartyA,
            BitString tPublicKeyPartyA,
            BitString tPublicKeyPartyB,
            BitString p,
            BitString q,
            BitString expectedSharedZ
        )
        {
            var result = _subject.GenerateSharedSecretZ(
                new FfcDomainParameters(p, q, 0.ToBitString()),
                new FfcKeyPair(xPrivateKeyPartyA, 0.ToBitString()),
                new FfcKeyPair(yPublicKeyPartyB),
                new FfcKeyPair(rPrivateKeyPartyA, 0.ToBitString()),
                new FfcKeyPair(tPublicKeyPartyA),
                new FfcKeyPair(tPublicKeyPartyB)
            );

            Assume.That(result.Success, $"{nameof(result)} should have been successful");
            Assert.AreEqual(expectedSharedZ, result.SharedSecretZ, nameof(expectedSharedZ));
        }
    }
}
...
namespace NIST.CVP.Generation.SSH.Tests
{
    ...
    public class TestCaseTests
    {
        ...
        public void ShouldReconstituteTestCaseFromProperJObject()
        {
            ...
            sourceTest.Add("initialIvClient", new JValue("00AA"));
            var subject = new TestCase(sourceTest);
            Assert.IsNotNull(subject);
        }
        ...
        [Test]
        public void ShouldSetProperEncryptionKeyServerFromDynamicAnswerTest()
        {
            var sourceTest = GetSourceAnswerTest();
            var subject = new TestCase(sourceTest);
            Assume.That(subject != null);
            Assert.AreEqual(sourceTest.encryptionKeyServer, subject.EncryptionKeyServer);
        }

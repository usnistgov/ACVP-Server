...
namespace NIST.CVP.Crypto.AES_CFB8.Tests
{
    ...
    {
        private Mock<IAES_CFB8> _aes;
        private AES_CFB8_MCT _subject;

        [SetUp]
        public void Setup()
        {
            _aes = new Mock<IAES_CFB8>();
            _subject = new AES_CFB8_MCT(_aes.Object);
        }
        ...
        public void ShouldRunEncryptOperation100000TimesForTestCase(int keySize)
        {
            ...
            BitString plainText = new BitString(8);
            _aes
                .Setup(s => s.BlockEncrypt(iv, key, plainText))
                .Returns(new EncryptionResult(new BitString(8)));

            var result = _subject.MCTEncrypt(iv, key, plainText);

            Assert.IsTrue(result.Success, nameof(result.Success));
            _aes.Verify(v => v.BlockEncrypt(It.IsAny<BitString>(), It.IsAny<BitString>(), It.IsAny<BitString>()), Times.Exactly(100000), nameof(_aes.Object.BlockEncrypt));
        }
        ...
        public void ShouldRunDecryptOperation100000TimesForTestCase(int keySize)
        {
            BitString iv = new BitString(128);
            BitString key = new BitString(keySize);
            BitString cipherText = new BitString(8);
            _aes
                .Setup(s => s.BlockDecrypt(iv, key, cipherText))
                .Returns(new DecryptionResult(new BitString(8)));

            var result = _subject.MCTDecrypt(iv, key, cipherText);

            Assert.IsTrue(result.Success, nameof(result.Success));
            _aes.Verify(v => v.BlockDecrypt(It.IsAny<BitString>(), It.IsAny<BitString>(), It.IsAny<BitString>()), Times.Exactly(100000), nameof(_aes.Object.BlockDecrypt));
        }

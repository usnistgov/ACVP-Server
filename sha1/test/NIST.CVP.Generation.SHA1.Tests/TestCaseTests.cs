...
namespace NIST.CVP.Generation.SHA1.Tests
{
    public class TestCaseTests
    {
        ...
        public void ShouldOnlyMergeWhenOriginalDigestOrMessageIsNullAndIsSuppliedByOther(string originalMessage, string suppliedMessage, string originalDigest, string suppliedDigest, bool expectedResult)
        {
            // Arrange
            var testCase = new TestCase { TestCaseId = 1 };
            SetBitString(testCase, "digest", originalDigest);
            SetBitString(testCase, "message", originalMessage);

            var suppliedTestCase = new TestCase { TestCaseId = 1 };
            SetBitString(suppliedTestCase, "digest", suppliedDigest);
            SetBitString(suppliedTestCase, "message", suppliedMessage);

...
namespace NIST.CVP.Generation.AES_CTR
{
    public class TestCaseGeneratorCounterEncrypt : ITestCaseGenerator<TestGroup, TestCase>
    {
        ...
        public TestCaseGenerateResponse Generate(TestGroup group, bool isSample)
        {
            if (isSample)
            {
                _numberOfMessages = 100;
            }

            var plainText = _rand.GetRandomBitString(128 * _numberOfMessages);
            var key = _rand.GetRandomBitString(128);
            var iv = GetStartingIV(group);
            //var iv = _rand.GetRandomBitString(128);     // TODO configure this for the group
        ...
        {
            var fillWithZeroes = true;
            var randomBits = 0;

            if ((group.TestType.ToLower() == "overflowcounter" && group.IncrementalCounter) ||
                (group.TestType.ToLower() == "counter" && !group.IncrementalCounter))
            {
                fillWithZeroes = false;
                randomBits = (int)System.Math.Log(_numberOfMessages) - 1;      // Intentional cast to toss away decimal
            }

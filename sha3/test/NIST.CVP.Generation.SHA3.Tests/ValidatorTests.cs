    ...
    public class ValidatorTests
    {
        ...
        public void ShouldValidateGoodResults()
        {
            ...
            var result = subject.Validate(Path.Combine(testPath, "result.json"), Path.Combine(testPath, "answer.json"),
                Path.Combine(testPath, "prompt.json"));
            Assert.IsNotNull(result);
        ...
        private Validator GetSubject(bool parserFail, bool validationFail)
        {
            ...
            if (validationFail)
            {
                mocks.MockIResultValidator
                    .Setup(
                        s =>
                            s.ValidateResults(It.IsAny<List<ITestCaseValidator<TestCase>>>(), It.IsAny<List<TestCase>>()))
                    .Returns(new TestVectorValidation
                    {
                        Validations = new List<TestCaseValidation> { new TestCaseValidation { Result = "failed" } }
                    });
            }
            else
            {
                mocks.MockIResultValidator
                    ...
                    .Returns(new TestVectorValidation
                    {
                        Validations = new List<TestCaseValidation> { new TestCaseValidation { Result = "passed" } }
                    });
            }
        ...
        {
            var vectorSet = new TestVectorSet { Algorithm = "SHA3" };
            ...
            var testGroups = _tdm.GetTestGroups(hashFunction, groups);
        ...
        {
            public Mock<IResultValidator<TestCase>> MockIResultValidator { get; set; } =
                new Mock<IResultValidator<TestCase>>();

            public Mock<IDynamicParser> MockIDynamicParser { get; set; } = new Mock<IDynamicParser>();

            public Mock<ITestCaseValidatorFactory<TestVectorSet, TestCase>> MockITestCaseValidatorFactory { get; set; }
                = new Mock<ITestCaseValidatorFactory<TestVectorSet, TestCase>>();
        }
    }
}

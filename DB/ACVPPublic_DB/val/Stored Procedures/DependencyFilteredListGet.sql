CREATE PROCEDURE [val].[DependencyFilteredListGet]
				...
				if (@PropertyID = 0)
					BEGIN
						...
						WHERE 1 = CASE
									...
									WHEN @PropertyName = 'type' AND @Operator = 'eq' AND dependency_type = @Value THEN 1
									WHEN @PropertyName = 'type' AND @Operator = 'contains' AND dependency_type LIKE '%' + @Value + '%' THEN 1
									WHEN @PropertyName = 'type' AND @Operator = 'start' AND dependency_type LIKE @Value + '%' THEN 1
									WHEN @PropertyName = 'type' AND @Operator = 'end' AND dependency_type LIKE '%' + @Value THEN 1
									WHEN @PropertyName = 'description' AND @Operator = 'eq' AND description = @Value THEN 1
									WHEN @PropertyName = 'description' AND @Operator = 'contains' AND description LIKE '%' + @Value + '%' THEN 1
									WHEN @PropertyName = 'description' AND @Operator = 'start' AND description LIKE @Value + '%' THEN 1
									WHEN @PropertyName = 'description' AND @Operator = 'end' AND description LIKE '%' + @Value THEN 1
									ELSE 0
								  END
					END
				ELSE
...
FROM val.VALIDATION_OE_DEPENDENCY_ATTRIBUTE A
	INNER JOIN
	@PageIDs X ON X.id = A.validation_oe_dependency_id

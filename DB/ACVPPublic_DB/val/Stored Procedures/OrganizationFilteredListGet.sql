CREATE PROCEDURE [val].[OrganizationFilteredListGet]
				...
				if (@PropertyID = 0)
					BEGIN
						...
						WHERE 1 = CASE
									...
									WHEN @PropertyName = 'website' AND @Operator = 'eq' AND organization_url = @Value THEN 1
									WHEN @PropertyName = 'website' AND @Operator = 'contains' AND organization_url LIKE '%' + @Value + '%' THEN 1
									WHEN @PropertyName = 'website' AND @Operator = 'start' AND organization_url LIKE @Value + '%' THEN 1
									WHEN @PropertyName = 'website' AND @Operator = 'end' AND organization_url LIKE '%' + @Value THEN 1
									WHEN @PropertyName = 'phoneNumber' AND @Operator = 'eq' AND (voice_number = @Value OR fax_number = @Value) THEN 1
									WHEN @PropertyName = 'phoneNumber' AND @Operator = 'contains' AND (voice_number LIKE '%' + @Value + '%' OR fax_number LIKE '%' + @Value + '%') THEN 1
									WHEN @PropertyName = 'phoneNumber' AND @Operator = 'start' AND (voice_number LIKE @Value + '%' OR fax_number LIKE @Value + '%') THEN 1
...
FROM val.[ADDRESS] A
	INNER JOIN
	@PageIDs X ON X.id = A.organization_id
...
FROM val.ORGANIZATION_EMAIL E
	INNER JOIN
	@PageIDs X ON X.id = E.organization_id

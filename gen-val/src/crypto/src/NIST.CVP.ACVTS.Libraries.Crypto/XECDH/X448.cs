...
{
    public class X448 : XecdhBase
    {
        public X448(Curve curveName, BigInteger p, BigInteger g, int bits, int a24, IEntropyProvider entropyProvider) :
        ...
        public override XecdhKeyPairGenerateResult DeriveKeyPair(BitString privateKey)
        {
            ...
            if (privateKey.BitLength != 8 * 56)
            {
                return new XecdhKeyPairGenerateResult("Private key must be 56 bytes long");
            }
        ...
        {
            // Section 6.2 of RFC 7748.
            var privateKey = _entropyProvider.GetEntropy(8 * 56);
        ...
        {
            // Section 6.2 of RFC 7748.
            if (keyPair.PublicKey.BitLength != 8 * 56)
            {
                return new XecdhKeyPairValidateResult("Public key must be 56 bytes long");
            }
            ...
            // No pair-wise consistency test, accepting any 56 bytes as a public key

...
/// <summary>
/// Implements the functions F(), H(), and Tl() from FIPS 205 section 10.2. Slightly generalized,
/// F(PK.seed,ADRS,M) = H(PK.seed,ADRS,M) = Tl(PK.seed,ADRS,M) = Truncn(SHA-256(PK.seed ∥ toByte(0,64−n) ∥ ADRSc ∥ M)) 
/// </summary>
public class FHOrTSha2SecurityCategory1 : IFHOrT
{
    private ISha _sha2_256;
    private int _n;
    
    public FHOrTSha2SecurityCategory1(int n, IShaFactory shaFactory)
    {
        _n = n;
    ...
    /// <summary>
    /// Performs F(PK.seed,ADRS,M) = H(PK.seed,ADRS,M) = Tl(PK.seed,ADRS,M) = Truncn(SHA-256(PK.seed ∥ toByte(0,64−n) ∥ ADRSc ∥ M)) 
    /// </summary>

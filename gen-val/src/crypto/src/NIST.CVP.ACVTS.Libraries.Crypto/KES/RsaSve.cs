...
namespace NIST.CVP.ACVTS.Libraries.Crypto.KES
{
    public class RsaSve : IRsaSve
    {
        ...
        public SharedSecretWithEncryptedValueResponse Generate(PublicKey rsaPublicKey)
        {
            ...
            var nLen = rsaPublicKey.N.ExactBitLength().ValueToMod(BitString.BITSINBYTE);
        ...
        public SharedSecretResponse Recover(KeyPair rsaKeyPair, BitString ciphertext)
        {
            ...
            var nLen = rsaKeyPair.PubKey.N.ExactBitString().BitLength.ValueToMod(BitString.BITSINBYTE);

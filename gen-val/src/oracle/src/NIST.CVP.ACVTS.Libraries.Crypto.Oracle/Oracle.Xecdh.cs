...
namespace NIST.CVP.ACVTS.Libraries.Crypto.Oracle
{
    public partial class Oracle
    {
        ...
        public async Task<XecdhSscResult> GetDeferredXecdhSscTestAsync(XecdhSscParameters param)
        {
            try
            {
                var observableGrain =
                    await GetObserverGrain<IOracleObserverDeferredXecdhSscCaseGrain, XecdhSscResult>();
                await GrainInvokeRetryWrapper.WrapGrainCall(observableGrain.Grain.BeginWorkAsync, param, LoadSheddingRetries);

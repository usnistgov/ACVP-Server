...
namespace NIST.CVP.ACVTS.Libraries.Generation.Tests.SNMP
{
    ...
    public class TestCaseValidatorTests
    {
        ...
        public async Task ShouldFailIfValueDoesNotMatch()
        {
            ...
            Assert.That(result.Reason.Contains(nameof(suppliedResult.SharedKey)), Is.True);
        }
        ...
        public async Task ShouldFailIfValueNotPresent()
        {
            ...
            Assert.That(result.Reason.Contains($"{nameof(suppliedResult.SharedKey)} was not present in the {nameof(TestCase)}"), Is.True);
        }

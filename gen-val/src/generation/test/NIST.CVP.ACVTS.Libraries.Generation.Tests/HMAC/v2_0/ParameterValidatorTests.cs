...
namespace NIST.CVP.ACVTS.Libraries.Generation.Tests.HMAC.v2_0
{
    ...
    public class ParameterValidatorTests
    {
        ...
        [TestCase("test1 - at minimum", ParameterValidator.MinKeyLength, true)]
        [TestCase("test2 - not mod 8", 15, false)]
        [TestCase("test3 - at max", ParameterValidator.MaxKeyLength, true)]
        ...
        [Test]
        [TestCase("test0 - below minimum", -8, false)]
        [TestCase("test1 - at minimum", ParameterValidator.MinMsgLength, true)]
        [TestCase("test2 - above maximum", 65536, false)]
        [TestCase("test3 - at max", ParameterValidator.MaxMsgLength, true)]
        public void ShouldSucceedOnValidMessageLen(string label, int messageLen, bool isSuccessExpected)
        {
            // SHA flavor does not matter for keylen
            Parameters parameters = new ParameterBuilder()
                .WithMessageLen(new MathDomain().AddSegment(new ValueDomainSegment(messageLen)))
                .Build();

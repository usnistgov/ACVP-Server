...
using NIST.CVP.ACVTS.Libraries.Generation.TDES_CBCI.v1_0.Parsers;
using NIST.CVP.ACVTS.Tests.Core;
using NUnit.Framework;
using MonteCarloKeyMaker = NIST.CVP.ACVTS.Libraries.Crypto.TDES_CBCI.MonteCarloKeyMaker;
                    ...
                    else
                    {
                        ...
                        if (testGroup.Function.ToLower() == "encrypt")
                        {
                            if (testGroup.TestType.ToLower() == "inversepermutation")
                            {
                                ...
                                Assert.That(result.Result.ToString(), Is.EqualTo(testCase.CipherText.ToString()),
                            ...
                            else if (testGroup.TestType.ToLower() == "multiblockmessage")
                            {
                                ...
                                Assert.That(result.Result.ToString(), Is.EqualTo(testCase.CipherText.ToString()), $"Failed on count {count} expected CT {testCase.CipherText}, got { result.Result}");
                                continue;
                            }
                        ...
                        if (testGroup.Function.ToLower() == "decrypt")
                        {
                            ...
                            else if (testGroup.TestType.ToLower() == "multiblockmessage")
                            {
                                ...
                                Assert.That(result.Result.ToString(), Is.EqualTo(testCase.PlainText.ToString()), $"Failed on count {count} expected CT {testCase.PlainText}, got {result.Result}");
                                continue;
                            }
                            else if (testGroup.TestType.ToLower() == "permutation" ||
                                     testGroup.TestType.ToLower() == "substitutiontable" ||

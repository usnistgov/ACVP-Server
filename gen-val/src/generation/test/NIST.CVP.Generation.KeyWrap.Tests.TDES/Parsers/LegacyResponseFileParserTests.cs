...
namespace NIST.CVP.Generation.KeyWrap.Tests.TDES.Parsers
{
    ...
    public class LegacyResponseFileParserTests
    {
        ...
        private const string _FOLDER_NAME = "TDESAAA";
...
";
        ...
        public void Setup()
        {
            ...
            File.WriteAllText($@"{_unitTestPath}\TKW_AD{_FILE_EXTENSION}", _testFIleContents); // 5 groups, 5 tests per group, 25 tests total
            File.WriteAllText($@"{_unitTestPath}\TKW_AE_inv{_FILE_EXTENSION}", _testFIleContents); // + 5 groups, 5 tests per group, 25 tests total
        ...
        public void ShouldParseKeyLenProperlyForGroups()
        {
            ...
            var vectorSet = result.ParsedObject;
            var testGroups = vectorSet.TestGroups.Select(s => (TestGroup)s);
            Assert.AreEqual(expectedNumberOfGroups, testGroups.Count(c => c.KeyLength == 192), "keyLen 192");
        }

...
{
    public class TestCaseValidatorMctSig : ITestCaseValidatorAsync<TestGroup, TestCase>
    {
        private readonly TestCase _expectedResult;

        public int TestCaseId => _expectedResult.TestCaseId;

        public TestCaseValidatorMctSig(TestCase expectedResult)
        {
        ...
        private void ValidateArrayResultPresent(TestCase suppliedResult, List<string> errors)
        {
            ...
            if (suppliedResult.ResultsArray.Count != _expectedResult.ResultsArray.Count)
            {
                errors.Add($"{nameof(suppliedResult.ResultsArray)} was incorrect length in the {nameof(TestCase)}");
            }

            if (suppliedResult.ResultsArray.Any(a => a.Signature == null))
            {
                errors.Add($"{nameof(suppliedResult.ResultsArray)} did not contain expected element {nameof(AlgoArrayResponse.Signature)}");
            }
        }
        ...
        private void CheckResults(TestCase suppliedResult, List<string> errors, Dictionary<string, string> expected, Dictionary<string, string> provided)
        {
            ...
            {
                if (!_expectedResult.ResultsArray[i].Signature.Equals(suppliedResult.ResultsArray[i].Signature))
                {
                    errors.Add($"Signature does not match on iteration {i}");
                    expected.Add($"Signature {i}", _expectedResult.ResultsArray[i].Signature.ToHex());
                    provided.Add($"Signature {i}", suppliedResult.ResultsArray[i].Signature.ToHex());
                }
            }
        }
    }

    ...
    public class TestGroupGenerator : ITestGroupGeneratorAsync<Parameters, TestGroup, TestCase>
    {
                    ...
                    foreach (var parameterSet in hashPerParameterSet)
                    {
                        if (parameterSet.Value.Any())
                        {
                            groups.Add(new TestGroup()
                            {
                                ...
                                IdIut = testType.Equals("val", StringComparison.OrdinalIgnoreCase) ? SpecificationMapping.IutId : null,
                                IdIutLen = testType.Equals("val", StringComparison.OrdinalIgnoreCase) ? SpecificationMapping.IutId.BitLength : 0,
                                TestType = testType,
        ...
        private IEnumerable<MacOptionsBase> GetMacAlgsPerParameterSet(ParameterSetBase parameterSet)
        {
            ...
            if (parameterSet != null)
            {
                ...
                macOptions.AddIfNotNull(parameterSet.MacOption.HmacSha1);

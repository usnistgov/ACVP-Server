        ...
        public async Task<TestCaseGenerateResponse<TestGroup, TestCase>> GenerateAsync(TestGroup group, bool isSample, int caseNo = 0)
        {
            ...
            try
            {
                if (group.MctVersion == MctVersions.Alternate)
                {
                    ...
                    var values = group.MessageLength.GetValues(x => x >= 128 && x <= 1024, 1, true);
                    
                    // If no values within the message domain that size, we get a wider breadth of numbers
                    if (!values.Any()) 
                    {
                        values = group.MessageLength.GetValues(x => x >= 32 && x <= 4096, 1, true);
                    }
                    ...
                    {
                        param.MessageLength = group.MessageLength.GetDomainMinMax().Maximum;
                    }

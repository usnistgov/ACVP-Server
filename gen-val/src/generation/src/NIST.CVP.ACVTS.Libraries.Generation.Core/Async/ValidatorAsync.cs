        ...
        where TTestCase : ITestCase<TTestGroup, TTestCase>
    {
        ...
        public async Task<ValidateResponse> ValidateAsync(ValidateRequest validateRequest)
        {
            ...
            try
            {
                ...
                response = await ValidateWorker(validateRequest.ResultJson, validateRequest.InternalJson,
                    validateRequest.ShowExpected);
            }
            ...
            {
                ThisLogger.Error(ex, "ERROR in Validator. NRE");
                return new ValidateResponse("General exception. Contact service provider.", StatusCode.TestCaseValidatorError);
            }
            catch (Exception ex)
            {

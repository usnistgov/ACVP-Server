        ...
        public GenerateResponse BuildTestCases(TestVectorSet testVectorSet)
        {
            ...
            foreach (var group in testVectorSet.TestGroups.Select(g => (TestGroup)g))
            {
                if (group.NumberOfKeys == 1)
                {
                    ...
                    var kats = _iKnownAnswerTestFactory.GetKATTestCases(group.TestType, group.Direction);

                    // Decrypt kats already have TestCaseId set. Decrypt and Encrypt are not separate
                    if (kats.Count == 0)
                    {
                        return new GenerateResponse($"Found 0 {group.Direction}: {group.TestType} tests");
                    }

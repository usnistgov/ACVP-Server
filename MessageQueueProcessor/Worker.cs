		...
		protected override async Task ExecuteAsync(CancellationToken stoppingToken)
		{
			...
			while (!stoppingToken.IsCancellationRequested)
			{
				...
				while (message != null && !stoppingToken.IsCancellationRequested)
				{
					...
					IMessageProcessor messageProcessor = _messageProcessorFactory.GetMessageProcessor(message.APIAction);
					...
					else
					{
						...
						if (messageProcessingResult.IsSuccess)
						{
							...
							//Log it
							_logger.LogInformation($"Processed {message.APIAction.ToString()} message {message.ID}");
						}
						else
						{
							//Mark it as an error
							...
							//Log it
							_logger.LogError($"Error processing {message.APIAction.ToString()} message {message.ID} : {messageProcessingResult.ErrorMessage}");
						}
					}

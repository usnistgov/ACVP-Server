...
{
	public class DeleteOrganizationProcessor : IMessageProcessor
	{
		private readonly IWorkflowService _workflowService;
		private readonly IWorkflowItemProcessorFactory _workflowItemProcessorFactory;
		private readonly bool _autoApprove;

		public DeleteOrganizationProcessor(IWorkflowService workflowService, IWorkflowItemProcessorFactory workflowItemProcessorFactory, bool autoApprove)
		{
		...
		public void Process(Message message)
		{
			...
			WorkflowInsertResult workflowInsertResult = _workflowService.AddWorkflowItem(APIAction.DeleteVendor, requestPayload.RequestID, requestPayload.Json.ToString(), requestPayload.UserID);
			...
			if (workflowInsertResult.IsSuccess && _autoApprove)
			{
				...
				WorkflowItem workflowItem = new WorkflowItem
				{
					...
					APIAction = APIAction.DeleteVendor,
					JSON = requestPayload.Json.ToString()
				};

				//Get the processor for this workflow item
				IWorkflowItemProcessor workflowItemProcessor = _workflowItemProcessorFactory.GetWorkflowItemProcessor(APIAction.DeleteVendor);

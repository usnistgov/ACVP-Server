...
{
	public class HMAC_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "DRBG800-90A_Summary.txt";
		private string _summaryFilePath;

		public HMAC_ResultsExtractor(string submissionPath)
		{
		...
		public HMAC_Results Extract()
		{
			...
			{
				//Skip everything that isn't the HMAC mode header
				if (!lines[i].StartsWith("[HMAC_DRBG")) continue;

				//Get the appropriate lines of results
				results.Results = GetTestResults(lines, i, AlgorithmTestNames.DRBG_HMAC);

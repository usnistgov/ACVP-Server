...
{
	public class TDES_CFBP64_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "TDES_Summary.txt";
		private string _summaryFilePath;

		public TDES_CFBP64_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public TDES_CFBP64_Results Extract()
		{
			TDES_CFBP64_Results results = new TDES_CFBP64_Results();
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				//Skip everything that isn't a CFBP64 mode header
				if (!line.StartsWith("[Mode CFBP64")) continue;

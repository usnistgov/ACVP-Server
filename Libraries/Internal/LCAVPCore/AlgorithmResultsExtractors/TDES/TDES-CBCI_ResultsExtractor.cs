...
{
	public class TDES_CBCI_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "TDES_Summary.txt";
		private string _summaryFilePath;

		public TDES_CBCI_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public TDES_CBCI_Results Extract()
		{
			TDES_CBCI_Results results = new TDES_CBCI_Results();
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				//Skip everything that isn't a CBCI mode header
				if (!line.StartsWith("[Mode CBCI")) continue;

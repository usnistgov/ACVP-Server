...
{
	public class TDES_CFB1_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "TDES_Summary.txt";
		private string _summaryFilePath;

		public TDES_CFB1_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public TDES_CFB1_Results Extract()
		{
			TDES_CFB1_Results results = new TDES_CFB1_Results();
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				//Skip everything that isn't a CFB1 mode header
				if (!line.StartsWith("[Mode CFB1")) continue;

...
namespace LCAVPCore.Registration.Algorithms.KDF
{
	public class KDF : AlgorithmBase, IAlgorithm
	{
		...
		public IPersistedAlgorithm ToPersistedAlgorithm() => new NIST.CVP.Algorithms.Persisted.KDF
		{
			Capabilities = Capabilities.Select(x => new NIST.CVP.Algorithms.Persisted.KDF.Capability
			{
				KDFMode = x.Mode,
				MacMode = x.MACModes,

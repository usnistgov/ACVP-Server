...
namespace LCAVPCore
{
	public static class ExtensionMethods
	{
		...
		public static NIST.CVP.Algorithms.DataTypes.Domain ToCoreDomain(List<int> values)
		{
			var domain = new NIST.CVP.Algorithms.DataTypes.Domain();

			foreach(int value in values)
			{
				domain.Segments.Add(new NIST.CVP.Algorithms.DataTypes.NumericSegment { Value = value });
			}

			return domain;
		}

		public static List<NIST.CVP.Algorithms.DataTypes.Range> ToCoreRangeList(List<Range> values) => values.Select(x => x.ToCoreRange()).ToList();
	}
}
...
{
	public class TDES_CFB64_Evaluator : AlgorithmEvaluatorBase, IAlgorithmEvaluator
	{
		public TDES_CFB64_Evaluator(Dictionary<string, string> options, string submissionPath) : base(options, submissionPath)
		{
		}
		...
		public AlgorithmEvaluationResult Evaluate()
		{
			...
			var resultExtractor = new TDES_CFB64_ResultsExtractor(SubmissionPath);
			...
			else
			{
				...
				int keyingOption = Options.GetValue("CFB_KEY_CHOICE1") == "Yes" ? 1 : 2;

				//Verify the appropriate tests based on the state
				switch (Options.GetValue("CFB64_State"))
				{
					case "Decrypt":
					...
					default:
						algoResult.AddFailure("Invalid CFB64 State");
						break;
				}
			}

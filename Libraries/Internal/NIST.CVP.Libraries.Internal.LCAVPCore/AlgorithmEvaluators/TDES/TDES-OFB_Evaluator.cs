...
{
	public class TDES_OFB_Evaluator : AlgorithmEvaluatorBase, IAlgorithmEvaluator
	{
		public TDES_OFB_Evaluator(Dictionary<string, string> options, string submissionPath) : base(options, submissionPath)
		{
		}
		...
		public AlgorithmEvaluationResult Evaluate()
		{
			...
			var resultExtractor = new TDES_OFB_ResultsExtractor(SubmissionPath);
			...
			else
			{
				...
				int keyingOption = Options.GetValue("OFB_KEY_CHOICE1") == "Yes" ? 1 : 2;

				//Verify the appropriate tests based on the state
				switch (Options.GetValue("OFB_State"))
				{
					case "Decrypt":
					...
					default:
						algoResult.AddFailure("Invalid OFB State");
						break;
				}
			}

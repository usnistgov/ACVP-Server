...
{
	public class TDES_CFB8_Evaluator : AlgorithmEvaluatorBase, IAlgorithmEvaluator
	{
		public TDES_CFB8_Evaluator(Dictionary<string, string> options, string submissionPath) : base(options, submissionPath)
		{
		}
		...
		public AlgorithmEvaluationResult Evaluate()
		{
			...
			var resultExtractor = new TDES_CFB8_ResultsExtractor(SubmissionPath);
			...
			else
			{
				...
				switch (Options.GetValue("CFB8_State"))
				{
					case "Decrypt":
					...
					default:
						algoResult.AddFailure("Invalid CFB8 State");
						break;
				}
			}

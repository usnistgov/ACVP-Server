...
{
	public class TDES_CBC_Evaluator : AlgorithmEvaluatorBase, IAlgorithmEvaluator
	{
		public TDES_CBC_Evaluator(Dictionary<string, string> options, string submissionPath) : base(options, submissionPath)
		{
		}
		...
		public AlgorithmEvaluationResult Evaluate()
		{
			...
			var resultExtractor = new TDES_CBC_ResultsExtractor(SubmissionPath);
			...
			else
			{
				...
				int keyingOption = Options.GetValue("CBC_KEY_CHOICE1") == "Yes" ? 1 : 2;

				//Verify the appropriate tests based on the state
				switch (Options.GetValue("CBC_State"))
				{
					case "Decrypt":
					...
					default:
						algoResult.AddFailure("Invalid CBC State");
						break;
				}
			}

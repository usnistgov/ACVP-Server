...
{
	public class TDES_ECB_Evaluator : AlgorithmEvaluatorBase, IAlgorithmEvaluator
	{
		public TDES_ECB_Evaluator(Dictionary<string, string> options, string submissionPath) : base(options, submissionPath)
		{
		}
		...
		public AlgorithmEvaluationResult Evaluate()
		{
			...
			var resultExtractor = new TDES_ECB_ResultsExtractor(SubmissionPath);
			...
			else
			{
				...
				int keyingOption = Options.GetValue("ECB_KEY_CHOICE1") == "Yes" ? 1 : 2;

				//Verify the appropriate tests based on the state
				switch (Options.GetValue("ECB_State"))
				{
					case "Decrypt":
					...
					default:
						algoResult.AddFailure("Invalid ECB State");
						break;
				}
			}

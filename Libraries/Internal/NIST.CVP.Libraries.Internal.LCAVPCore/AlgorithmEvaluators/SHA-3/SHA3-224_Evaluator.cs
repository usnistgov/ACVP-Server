...
{
	public class SHA3_224_Evaluator : AlgorithmEvaluatorBase, IAlgorithmEvaluator
	{
		public SHA3_224_Evaluator(Dictionary<string, string> options, string submissionPath) : base(options, submissionPath)
		{
		}

		public AlgorithmEvaluationResult Evaluate()
		{
			var resultExtractor = new SHA3_224_ResultsExtractor(SubmissionPath);
			...
			else
			{
				...
				if (bool.Parse(Options.GetValue("SHA3_224_Byte")) != results.ByteOrientedOnly) algoResult.AddFailure("ByteOrientedOnly does not match");

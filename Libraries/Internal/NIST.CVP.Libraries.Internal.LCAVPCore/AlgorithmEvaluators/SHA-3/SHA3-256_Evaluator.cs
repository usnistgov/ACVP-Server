...
{
	public class SHA3_256_Evaluator : AlgorithmEvaluatorBase, IAlgorithmEvaluator
	{
		public SHA3_256_Evaluator(Dictionary<string, string> options, string submissionPath) : base(options, submissionPath)
		{
		}

		public AlgorithmEvaluationResult Evaluate()
		{
			var resultExtractor = new SHA3_256_ResultsExtractor(SubmissionPath);
			...
			else
			{
				...
				if (bool.Parse(Options.GetValue("SHA3_256_Byte")) != results.ByteOrientedOnly) algoResult.AddFailure("ByteOrientedOnly does not match");

...
{
	public class HMAC_SHA2_512256 : AlgorithmBase, IAlgorithm
	{
		...
		public HMAC_SHA2_512256(Dictionary<string, string> options, IDataProvider dataProvider) : base(dataProvider, "HMAC-SHA2-512/256")
		{
			...
			//keyLenValues.Add(8 * ParsingHelper.ParseValueToInteger(options.GetValue("HMAC_SHA512_256_K<B_1")));
			//keyLenValues.Add(8 * ParsingHelper.ParseValueToInteger(options.GetValue("HMAC_SHA512_256_K<B_2")));
			//keyLenValues.Add(8 * ParsingHelper.ParseValueToInteger(options.GetValue("HMAC_SHA512_256_K>B_1")));
			//keyLenValues.Add(8 * ParsingHelper.ParseValueToInteger(options.GetValue("HMAC_SHA512_256_K>B_2")));
			//if (options.GetValue("HMAC_SHA512_256_K_EQ_B") == "True") keyLenValues.Add(1024);

			//KeyLen.AddRange(keyLenValues.Where(x => x >= 8).Distinct().Cast<object>());

			KeyLessThanBlockSize = ParsingHelper.ParseValueToInteger(options.GetValue("HMAC_SHA512_256_K<B_1")) > 0 || ParsingHelper.ParseValueToInteger(options.GetValue("HMAC_SHA512_256_K<B_2")) > 0;
			KeyGreaterThanBlockSize = ParsingHelper.ParseValueToInteger(options.GetValue("HMAC_SHA512_256_K>B_1")) > 0 || ParsingHelper.ParseValueToInteger(options.GetValue("HMAC_SHA512_256_K>B_2")) > 0;
			KeyEqualBlockSize = options.GetValue("HMAC_SHA512_256_K_EQ_B") == "True";

			if (options.GetValue("HMAC_SHA512_256_MAC16") == "True") MacLen.Add(16 * 8);
			if (options.GetValue("HMAC_SHA512_256_MAC24") == "True") MacLen.Add(24 * 8);
			if (options.GetValue("HMAC_SHA512_256_MAC32") == "True") MacLen.Add(32 * 8);
		}

		public IPersistedAlgorithm ToPersistedAlgorithm() => new NIST.CVP.Libraries.Internal.Algorithms.Persisted.HMAC_SHA2_512256
		{

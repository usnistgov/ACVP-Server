	...
	public class RSALegacySigVer : AlgorithmBase, IAlgorithm
	{
		...
		public RSALegacySigVer(Dictionary<string, string> options, IDataProvider dataProvider) : base(dataProvider, "RSA", "legacySigVer")
		{
			...
			if (options.GetValue("Legacy_PKCS#1_PSSSigVer") == "True")
			{
				...
				List<HashPair> hashPairs = new List<HashPair>();
				if (options.GetValue("Legacy_PKCS#1_PSSSigVer_SHA1") == "True") hashPairs.Add(new HashPair { HashAlgorithm = "SHA-1", SaltLen = System.Math.Min(saltLen, 160) });
				if (options.GetValue("Legacy_PKCS#1_PSSSigVer_SHA224") == "True") hashPairs.Add(new HashPair { HashAlgorithm = "SHA2-224", SaltLen = System.Math.Min(saltLen, 224) });
				if (options.GetValue("Legacy_PKCS#1_PSSSigVer_SHA256") == "True") hashPairs.Add(new HashPair { HashAlgorithm = "SHA2-256", SaltLen = System.Math.Min(saltLen, 256) });
				if (options.GetValue("Legacy_PKCS#1_PSSSigVer_SHA384") == "True") hashPairs.Add(new HashPair { HashAlgorithm = "SHA2-384", SaltLen = System.Math.Min(saltLen, 384) });
				if (options.GetValue("Legacy_PKCS#1_PSSSigVer_SHA512") == "True") hashPairs.Add(new HashPair { HashAlgorithm = "SHA2-512", SaltLen = System.Math.Min(saltLen, 512) });
		...
		public IPersistedAlgorithm ToPersistedAlgorithm() => new NIST.CVP.Libraries.Internal.Algorithms.Persisted.RSASigVer186_2
		{
			Capabilities = Capabilities.Select(x => new NIST.CVP.Libraries.Internal.Algorithms.Persisted.RSASigVer186_2.Capability
			{

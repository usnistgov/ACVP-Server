...
{
	public class AES_CFB8 : AlgorithmBase, IAlgorithm
	{
		...
		public AES_CFB8(Dictionary<string, string> options, IDataProvider dataProvider) : base(dataProvider, "ACVP-AES-CFB8")
		{
			List<string> validValues = new List<string> { "Both", "Encrypt", "Decrypt" };
			KeyLen = new List<int>();
			if (validValues.Contains(options.GetValue("CFB8_128_State"))) KeyLen.Add(128);
			if (validValues.Contains(options.GetValue("CFB8_192_State"))) KeyLen.Add(192);
			if (validValues.Contains(options.GetValue("CFB8_256_State"))) KeyLen.Add(256);
		...
		public IPersistedAlgorithm ToPersistedAlgorithm() => new NIST.CVP.Libraries.Internal.Algorithms.Persisted.AES_CFB8
		{
			Direction = Direction,
			KeyLength = KeyLen
		};

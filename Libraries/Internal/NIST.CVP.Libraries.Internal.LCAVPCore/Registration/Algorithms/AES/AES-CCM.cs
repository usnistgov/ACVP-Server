...
namespace NIST.CVP.Libraries.Internal.LCAVPCore.Registration.Algorithms.AES
{
	public class AES_CCM : AlgorithmBase, IAlgorithm
	{
		...
		public AES_CCM(Dictionary<string, string> options, IDataProvider dataProvider) : base(dataProvider, "ACVP-AES-CCM")
		{
			...
			{
				PtLen.Add(new MathDomain.Range()
				{
					Min = ptLenMin,
					Max = ptLenMax
				});
			...
			int aadLenMax = System.Math.Max(ParsingHelper.ParseValueToInteger(options.GetValue("CCM_AdataMax")) * 8, options.GetValue("CCM_Adata216") == "True" ? 524288 : 0);      //Max of the max value provided or 2^16 * 8, if they checked that box
			...
			if (aadLenMin == aadLenMax)
			{
				...
				AadLen.Add(new MathDomain.Range()
				{
					Min = aadLenMin,
					Max = aadLenMax
				});
		...
		public IPersistedAlgorithm ToPersistedAlgorithm() => new NIST.CVP.Libraries.Internal.Algorithms.Persisted.AES_CCM
		{
			KeyLength = KeyLen,

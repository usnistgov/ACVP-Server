...
namespace NIST.CVP.Libraries.Internal.LCAVPCore.Registration.Algorithms.SHA_3
{
	public class SHAKE_128 : AlgorithmBase, IAlgorithm
	{
		...
		public SHAKE_128(Dictionary<string, string> options, IDataProvider dataProvider) : base(dataProvider, "SHAKE-128")
		{
			...
			{
				Min = System.Math.Max(ParsingHelper.ParseValueToInteger(options.GetValue("SHAKE128_Output_MinLen")), 16),		//Must be at least 16, so just in case something funny happened force it to 16
				Max = options.GetValue("SHAKE128_OutputMax2TO16") == "True" ? (int)System.Math.Pow(2, 16) : ParsingHelper.ParseValueToInteger(options.GetValue("SHAKE128_Output_MaxLen"))		//Use 2^16 if they checked the box, otherwise the number provided
			});
		}

		public IPersistedAlgorithm ToPersistedAlgorithm() => new NIST.CVP.Libraries.Internal.Algorithms.Persisted.SHAKE_128
		{

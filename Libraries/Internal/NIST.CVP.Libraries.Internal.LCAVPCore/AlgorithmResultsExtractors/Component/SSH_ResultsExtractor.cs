...
{
	public class SSH_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "kdf135_Summary.txt";
		private string _summaryFilePath;

		public SSH_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public SSH_Results Extract()
		{
			SSH_Results results = new SSH_Results();
			...
			for (i = 0; i < lines.Length; i++)
			{
				...
				//Skip everything that isn't the SSH line
				if (!line.StartsWith("SSH KDF")) continue;

				//Found the SSH line, so the next line will be the result
				line = lines[i + 1];
				results.Passed = line == "\t\t\tPASSED";
				break;
			}

			//Handle error case of SSH line not being found
			if (i == lines.Length) results.InvalidReasons.Add("SSH result not found");

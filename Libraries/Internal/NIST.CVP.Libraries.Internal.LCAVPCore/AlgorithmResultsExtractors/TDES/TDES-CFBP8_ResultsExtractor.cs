...
{
	public class TDES_CFBP8_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "TDES_Summary.txt";
		private string _summaryFilePath;

		public TDES_CFBP8_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public TDES_CFBP8_Results Extract()
		{
			TDES_CFBP8_Results results = new TDES_CFBP8_Results();
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				//Skip everything that isn't a CFBP8 mode header
				if (!line.StartsWith("[Mode CFBP8")) continue;

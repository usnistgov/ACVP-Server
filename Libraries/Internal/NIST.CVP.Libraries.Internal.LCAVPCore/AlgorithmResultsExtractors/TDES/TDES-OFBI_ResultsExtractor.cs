...
{
	public class TDES_OFBI_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "TDES_Summary.txt";
		private string _summaryFilePath;

		public TDES_OFBI_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public TDES_OFBI_Results Extract()
		{
			TDES_OFBI_Results results = new TDES_OFBI_Results();
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				//Skip everything that isn't a OFBI mode header
				if (!line.StartsWith("[Mode OFBI")) continue;

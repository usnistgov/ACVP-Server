...
{
	public class AES_CBC_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "AES_Summary.txt";
		private string _summaryFilePath;

		public AES_CBC_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public AES_CBC_Results Extract()
		{
			AES_CBC_Results results = new AES_CBC_Results();
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				//Skip everything that isn't a CBC mode header
				if (!line.StartsWith("[Mode CBC")) continue;
		...
		private List<PassFailResult> GetTestResults(string[] lines, int stateLineIndex)
		{
			...
			//string[] testNames = {  "GF Substitution Box Known Answer Test",
			...
			//						"Monte Carlo Test"
			//};
			string[] testNames = AlgorithmTestNames.AES_CBC;
		...
		private List<PassFailResult> GetKeySizeActionResultObject(int keySize, string action, AES_CBC_Results results)
		{

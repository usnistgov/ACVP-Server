...
{
	public class AES_ECB_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "AES_Summary.txt";
		private string _summaryFilePath;

		public AES_ECB_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public AES_ECB_Results Extract()
		{
			AES_ECB_Results results = new AES_ECB_Results();
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				//Skip everything that isn't a ECB mode header
				if (!line.StartsWith("[Mode ECB")) continue;
		...
		private List<PassFailResult> GetTestResults(string[] lines, int stateLineIndex)
		{
			...
			string[] testNames = AlgorithmTestNames.AES_ECB;
		...
		private List<PassFailResult> GetKeySizeActionResultObject(int keySize, string action, AES_ECB_Results results)
		{

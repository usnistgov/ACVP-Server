...
{
	public class TDES_OFBI_Evaluator : AlgorithmEvaluatorBase, IAlgorithmEvaluator
	{
		public TDES_OFBI_Evaluator(Dictionary<string, string> options, string submissionPath) : base(options, submissionPath)
		{
		}
		...
		public AlgorithmEvaluationResult Evaluate()
		{
			...
			var resultExtractor = new TDES_OFBI_ResultsExtractor(SubmissionPath);
			...
			else
			{
				...
				int keyingOption = Options.GetValue("OFBI_KEY_CHOICE1") == "Yes" ? 1 : 2;

				//Verify the appropriate tests based on the state
				switch (Options.GetValue("OFBI_State"))
				{
					case "Decrypt":
					...
					default:
						algoResult.AddFailure("Invalid OFBI State");
						break;
				}
			}

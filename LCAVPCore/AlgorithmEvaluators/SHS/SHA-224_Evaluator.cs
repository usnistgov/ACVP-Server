...
{
	public class SHA_224_Evaluator : AlgorithmEvaluatorBase, IAlgorithmEvaluator
	{
		public SHA_224_Evaluator(Dictionary<string, string> options, string submissionPath) : base(options, submissionPath)
		{
		}

		public AlgorithmEvaluationResult Evaluate()
		{
			var resultExtractor = new SHA_224_ResultsExtractor(SubmissionPath);
			...
			else
			{
				...
				if (bool.Parse(Options.GetValue("SHA224_Byte")) != results.ByteOrientedOnly) algoResult.AddFailure("ByteOrientedOnly does not match");

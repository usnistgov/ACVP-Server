...
using LCAVPCore.AlgorithmResultsExtractors.SHS;

namespace LCAVPCore.AlgorithmEvaluators.SHS
{
	public class SHA_512224_Evaluator : AlgorithmEvaluatorBase, IAlgorithmEvaluator
	{
		public SHA_512224_Evaluator(Dictionary<string, string> options, string submissionPath) : base(options, submissionPath)
		{
		}

		public AlgorithmEvaluationResult Evaluate()
		{
			var resultExtractor = new SHA_512224_ResultsExtractor(SubmissionPath);
			...
			else
			{
				...
				if (bool.Parse(Options.GetValue("SHA512_224_Byte")) != results.ByteOrientedOnly) algoResult.AddFailure("ByteOrientedOnly does not match");

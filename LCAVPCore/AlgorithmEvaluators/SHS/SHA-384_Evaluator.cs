...
{
	public class SHA_384_Evaluator : AlgorithmEvaluatorBase, IAlgorithmEvaluator
	{
		public SHA_384_Evaluator(Dictionary<string, string> options, string submissionPath) : base(options, submissionPath)
		{
		}

		public AlgorithmEvaluationResult Evaluate()
		{
			var resultExtractor = new SHA_384_ResultsExtractor(SubmissionPath);
			...
			else
			{
				...
				if (bool.Parse(Options.GetValue("SHA384_Byte")) != results.ByteOrientedOnly) algoResult.AddFailure("ByteOrientedOnly does not match");

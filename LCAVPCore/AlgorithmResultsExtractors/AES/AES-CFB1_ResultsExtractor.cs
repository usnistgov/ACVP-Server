...
{
	public class AES_CFB1_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "AES_Summary.txt";
		private string _summaryFilePath;

		public AES_CFB1_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public AES_CFB1_Results Extract()
		{
			AES_CFB1_Results results = new AES_CFB1_Results();
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				//Skip everything that isn't a CFB1 mode header - but watch out for CFB1 being a substring of CFB128
				if (!line.StartsWith("[Mode CFB1") || line.StartsWith("[Mode CFB128")) continue;
		...
		private List<PassFailResult> GetTestResults(string[] lines, int stateLineIndex)
		{
			...
			string[] testNames = AlgorithmTestNames.AES_CFB1;
		...
		private List<PassFailResult> GetKeySizeActionResultObject(int keySize, string action, AES_CFB1_Results results)
		{

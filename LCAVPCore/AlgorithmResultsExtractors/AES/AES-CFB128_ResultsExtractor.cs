...
{
	public class AES_CFB128_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "AES_Summary.txt";
		private string _summaryFilePath;

		public AES_CFB128_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public AES_CFB128_Results Extract()
		{
			AES_CFB128_Results results = new AES_CFB128_Results();
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				//Skip everything that isn't a CFB128 mode header
				if (!line.StartsWith("[Mode CFB128")) continue;

				//Have a mode header line. Make sure it is good and get the key size
				if (line.Length < 19 || !int.TryParse(line.Substring(15, 3), out keySize))
				{
		...
		private List<PassFailResult> GetTestResults(string[] lines, int stateLineIndex)
		{
			...
			string[] testNames = AlgorithmTestNames.AES_CFB128;
		...
		private List<PassFailResult> GetKeySizeActionResultObject(int keySize, string action, AES_CFB128_Results results)
		{

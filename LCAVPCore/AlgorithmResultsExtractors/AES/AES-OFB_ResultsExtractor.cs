...
{
	public class AES_OFB_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "AES_Summary.txt";
		private string _summaryFilePath;

		public AES_OFB_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public AES_OFB_Results Extract()
		{
			AES_OFB_Results results = new AES_OFB_Results();
			...
			for (int i = 0; i < lines.Length; i++)
			{
				...
				//Skip everything that isn't a OFB mode header
				if (!line.StartsWith("[Mode OFB")) continue;
		...
		private List<PassFailResult> GetTestResults(string[] lines, int stateLineIndex)
		{
			...
			string[] testNames = AlgorithmTestNames.AES_OFB;
		...
		private List<PassFailResult> GetKeySizeActionResultObject(int keySize, string action, AES_OFB_Results results)
		{

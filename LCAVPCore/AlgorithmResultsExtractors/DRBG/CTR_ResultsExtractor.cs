...
using LCAVPCore.AlgorithmResults.DRBG;

namespace LCAVPCore.AlgorithmResultsExtractors.DRBG
{
	public class CTR_ResultsExtractor : IAlgorithmResultsExtractor
	{
		private const string SUMMARY_FILE_NAME = "DRBG800-90A_Summary.txt";
		private string _summaryFilePath;

		public CTR_ResultsExtractor(string submissionPath)
		{
			_summaryFilePath = Path.Combine(submissionPath, SUMMARY_FILE_NAME);
		}

		public CTR_Results Extract()
		{
			CTR_Results results = new CTR_Results();
			...
			{
				//Skip everything that isn't the CTR mode header
				if (!lines[i].StartsWith("[CTR_DRBG")) continue;

				//Get the appropriate lines of results
				results.Results = GetTestResults(lines, i, AlgorithmTestNames.DRBG_CTR);

...
namespace NIST.CVP.Generation.DRBG.Tests
{
    ...
    class TestCaseGeneratorReseedNoPredResistTests
    {
        private TestCaseGeneratorReseedNoPredResist _subject;
        ...
        public void Setup()
        {
            ...
            _subject = new TestCaseGeneratorReseedNoPredResist(_entropyProviderFactory.Object, _drbgFactory.Object);
        }

        /// <summary>
        /// For Reseed/NoPredResist, two generates are run and a reseed.  
        /// OtherInput should have three objects within it to convey this.
        /// </summary>
        [Test]
        public void ShouldContainThreeCountInOtherInput()
        {
            var vectorSet = DRBG.TestDataMother.GetSampleVectorSet(1, 1, true, false);

            var testGroup = (TestGroup)vectorSet.TestGroups[0];

            var result = _subject.Generate(testGroup, false);

            Assert.AreEqual(3, ((TestCase)result.TestCase).OtherInput.Count);
        }
